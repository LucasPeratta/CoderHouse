...
class MongodbContainer {
  ...
  async getUserbyId(id) {
    try {
      ...
      if (!user)
        throw new Error("NOT_FOUND: Datos incorrectos. Intente nuevamente.");
      return user;
    } catch (error) {
      console.log(error);
    }
  ...
  async save(obj) {
    try {
      ...
      const id = !allObjs.length
        ? 1
        : parseInt(allObjs[allObjs.length - 1].id) + 1;
      if (isNaN(id))
        throw new Error(
          "ID_ERR: No se pudo asignar id al documento. Por lo tanto su peticion no pudo ser guardada en la base de datos, por favor communiquese con soporte para solucionar el problema. support@mentirita.com"
        );
      obj.id = id.toString();
...
export const products = new MongodbContainer(
  mongoConectionStr,
  "ecommerce",
  "products"
);
export const messages = new MongodbContainer(
  mongoConectionStr,
  "ecommerce",
  "messages"
);
